...
const BrowseCard = ({ shift, onView, onClaim, isClaiming }) => {
  return (
    <article className="group overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="flex items-start justify-between p-6">
        ...
        <span className="px-3 py-1 rounded-full text-xs font-semibold">
          ...
          {shift.status ? (
            ...
            <span className={statusColors[shift.status] ?? "bg-slate-100 text-slate-700"}>
              {shift.status}
            </span>
      ...
      <div className="mt-8 flex items-center justify-between border-t border-slate-200 dark:border-slate-800 p-6">
        ...
        <button
          ...
          disabled={isClaiming}
          className="flex items-center gap-2 rounded-2xl bg-emerald-600 px-5 py-3 font-semibold text-white transition hover:bg-emerald-700 disabled:opacity-50"
        >
