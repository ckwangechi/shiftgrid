...
const ShiftDetailsDrawer = ({ shift, isOpen, onClose, onClaim, onCancel }) => {
      ...
      <div className="relative z-10 w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-3xl bg-white dark:bg-slate-950 shadow-2xl flex flex-col animate-scaleIn">
        <header className="flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={onClose}
              className="p-3 rounded-xl bg-red-100 dark:bg-red-900/40 text-red-600"
            >
              <X size={18} />
            </button>
          </div>
        ...
        <div className="overflow-y-auto p-6 space-y-6">
          ...
          <section className="rounded-3xl border border-slate-200 dark:border-slate-800 p-5">
            ...
            <p className="text-slate-600 dark:text-slate-300">
              {shift.description ||
                "Join this event team and complete assigned responsibilities professionally."}
            </p>
          </section>
          ...
          <section className="rounded-3xl border border-slate-200 dark:border-slate-800 p-5">
            ...
            <div className="flex flex-wrap gap-3">
              {["National ID", "Smartphone", "Punctuality", "Team Work"].map(
                (item) => (
                  <span
                    key={item}
                    className="px-4 py-2 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm"
                  >
                    ✓ {item}
                  </span>
                )
        ...
        <footer className="p-6 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950">
          {shift.claimed ? (
            <div className="flex flex-col gap-3">
              <button
                disabled
                className="w-full py-4 rounded-2xl bg-slate-400 text-white font-bold text-lg cursor-not-allowed"
              >
                Claimed
              </button>
              <button
                onClick={onCancel}
                className="w-full py-3 rounded-2xl border border-red-500/30 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 font-semibold transition hover:bg-red-600 hover:text-white hover:border-red-600 flex items-center justify-center gap-2"
              >
                <X size={18} />
                Cancel Shift
              </button>
            ...
            <button
              onClick={onClaim}
              className="w-full py-4 rounded-2xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg transition"
            >
              Claim Shift
            </button>
          )}
